...
.ld-inner{text-align:center;padding:0 30px}
.ld-flourish{display:block;width:130px;height:auto;margin:0 auto 22px;color:var(--gold)}
.ld-names{font-family:var(--serif);font-weight:500;font-size:clamp(1.5rem,7vw,2rem);color:var(--green-deep);line-height:1.35}
.ld-names span{font-family:var(--script);color:var(--gold);font-size:1.25em;display:inline-block;margin:0 .08em;vertical-align:-.05em}
.ld-bar{width:120px;height:1px;background:rgba(152,116,30,.22);margin:26px auto 0;overflow:hidden}
.ld-bar i{display:block;height:100%;width:38%;background:var(--gold);animation:ldbar 1.5s var(--ease) infinite}
@keyframes ldbar{0%{transform:translateX(-120%)}100%{transform:translateX(330%)}}
...
/* BGM TOGGLE */
.bgm{position:fixed;right:16px;bottom:16px;z-index:200;width:42px;height:42px;border-radius:50%;border:1px solid rgba(152,116,30,.5);background:rgba(251,247,238,.82);-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--gold);box-shadow:0 4px 16px rgba(0,0,0,.12);transition:transform .3s var(--ease)}
.bgm:active{transform:scale(.92)}
.bgm svg{width:19px;height:19px}
.bgm .on-ico{display:none}.bgm.playing .on-ico{display:block}.bgm.playing .off-ico{display:none}

@media(prefers-reduced-motion:reduce){
 #loader{display:none}.ld-bar i{animation:none}
...
<button id="bgm-toggle" class="bgm" aria-label="Toggle music">
 <svg class="off-ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5 6 9H3v6h3l5 4V5Z"/><path d="m16 9 5 6M21 9l-5 6"/></svg>
 <svg class="on-ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5 6 9H3v6h3l5 4V5Z"/><path d="M16 8.5a5 5 0 0 1 0 7M19 5.5a9 9 0 0 1 0 13"/></svg>
</button>
<audio id="bgm" loop preload="none"><source src="assets/music.mp3" type="audio/mpeg"></audio>
...
(function(){
 ...
 (function(){var ld=document.getElementById('loader');if(!ld)return;
  if(reduce){ld.remove();return;}
  var done=false,go=function(){if(done)return;done=true;ld.classList.add('gone');setTimeout(function(){ld.parentNode&&ld.remove();},1000);};
  ld.addEventListener('click',go);setTimeout(go,1900);})();

 /* background music — soft, opt-out, starts on first touch */
 (function(){var a=document.getElementById('bgm'),btn=document.getElementById('bgm-toggle');if(!a||!btn)return;
  var playing=false,failed=false;
  a.addEventListener('error',function(){failed=true;btn.style.display='none';});
  function ramp(to){var v=a.volume||0,s=(to-v)/16,n=0,t=setInterval(function(){v+=s;a.volume=Math.min(1,Math.max(0,v));if(++n>=16){a.volume=to;clearInterval(t);}},45);}
  function start(){if(failed)return;a.volume=0;var p=a.play();if(p&&p.then)p.then(function(){playing=true;btn.classList.add('playing');ramp(.38);}).catch(function(){});}
  function stop(){a.pause();playing=false;btn.classList.remove('playing');}
  btn.addEventListener('click',function(e){e.stopPropagation();playing?stop():start();});
  var first=function(){if(!playing&&!failed)start();window.removeEventListener('pointerdown',first);};
